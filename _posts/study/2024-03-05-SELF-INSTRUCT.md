...
- SELF-INSTRUCT 도입: 적은 수의 인간 레이블 데이터를 사용하여 사전 학습된 언어 모델을 지시 조정하는 반자동 과정이다.
- 과정 설명: 수동으로 작성된 작업의 초기 세트에서 시작하여 모델이 새 작업에 대한 지시를 생성하고, 이를 바탕으로 입력-출력 인스턴스를 생성하며, 낮은 품질의 지시를 필터링하는 반복적인 과정이다.
- 경험적 평가: GPT3에 SELF-INSTRUCT 프레임워크를 적용하여 약 52K의 지시와 82K의 인스턴스 입력 및 대상 출력을 생성한다.
- 성능 비교: GPT3SELF-INST는 기존 GPT3보다 크게 우수하며 InstructGPT001과 비슷한 성능을 보인다.
- 인간 평가 결과: GPT3SELF-INST는 광범위한 지시 따르기 능력을 보여주며, 공개적으로 사용 가능한 다른 지시 데이터 세트에서 학습된 모델을 능가한다.
- 기여도: 최소한의 인간 레이블 데이터로 지시 따르기 능력을 유도하는 방법을 소개하고, 그 효과를 광범위한 지시 조정 실험을 통해 입증한다. 또한 52K 지시와 미래의 지시-따르기 모델을 구축하고 평가하기 위한 수동으로 작성된 새로운 작업 세트의 대규모 합성 데이터 세트를 공개한다.
...
- GPT3SELF-INST와 다른 모델 비교: GPT3SELF-INST가 T0 및 T0 학습 세트에 미세 조정된 GPT3보다 우수한 성능을 보인다.
- InstructGPT001과의 성능 비교: GPT3SELF-INST가 InstructGPT001의 성능에 근접했다.
...
- GPT3SELF-INST의 성능: GPT3SELF-INST가 T0 또는 SUPERNI 데이터에 학습된 모델들을 큰 폭으로 능가하며 생성된 데이터의 가치를 입증한다.
- InstructGPT와의 비교: GPT3SELF-INST는 InstructGPT001의 성능에 근접하며, InstructGPT002와 InstructGPT003의 인상적인 지시 따르기 능력을 확인한다.
- 향후 연구 방향: 생성된 데이터의 품질을 개선하기 위해 인간 주석자 사용 또는 보상 모델 학습을 통한 더 나은 생성물 선택 가능성을 탐색한다.

### 4.5 Effect of Data Size and Quality
- 데이터 크기의 효과: 데이터 크기 증가에 따라 GPT3SELF-INST 모델의 지시 따르기 능력이 일관되게 개선되나, 16K 이후에는 개선이 적다.
...
- 인간 평가 결과: SELF-INSTRUCT로 조정된 GPT3이 기존 공개 지시 데이터셋 사용보다 훨씬 우수하며, InstructGPT001에 근접한 성능을 보인다.
